# Page: pixel-art-maker

## 概要
ドット絵（ピクセルアート）を作って、端末内に保存できるページです（まずはMVP）。  
将来的に、作ったキャラ/モノ/地形を他のゲームで使えるようにするための「共通アセット置き場」になります。

## 操作方法
（画面上のツールを選んで描きます）

### 範囲選択（NEW）
- ツールバーの **「範囲」** を選ぶと、キャンバス上をドラッグして **四角い範囲選択** ができます
- **コピー**: 範囲選択してから `Ctrl/Cmd + C`（またはツールバーの「コピー」）
- **切り取り**: 範囲選択してから `Ctrl/Cmd + X`（またはツールバーの「切取」）
- **貼り付け**: `Ctrl/Cmd + V`（またはツールバーの「貼付」）で **貼り付けモード** → 置きたい場所をクリックで確定（`Esc`でキャンセル）
- **移動**: 範囲選択した中をドラッグすると移動（離すと確定）
  - 矢印キーでも移動できます（`Shift`同時押しで8pxずつ）
- **削除**: 範囲選択して `Delete` / `Backspace`
- **解除**: `Esc`

### 色相変更（NEW）
- 編集画面の **「色相」** から、スライダーでプレビューできます
- **「適用」** を押すと、キャンバス上の色が実際に変わります（**もどす** で戻せます）

### 画像からドット化（NEW）
- 編集画面で **「画像を読み込む」** を押して、画像ファイルを選ぶと
  - キャンバスサイズに合わせて縮小（縦横比は維持・中央寄せ）
  - 自動でドット化して反映
- 上書き前に確認が出ます（**「もどす」** で元に戻せます）

## 仕様（MVP）
- **種類**: UI上は区別しません（内部の互換性のため、保存データは既存の `kind` を保持します）
- **サイズ**: **新規は 16×16**。編集画面の **「横+16」「縦+16」** でキャンバスを拡張（最大256）
- **ギャラリー**: **みんなの作品が一覧に出ます**（共通）
- **保存**: Supabase（DB）に保存（ネットワーク不調時はIndexedDBにローカル保存して後で読み込み可能）
- **書き出し**: PNGとしてダウンロード可能
- **他人の作品**: 読み込みはできますが **読み取り専用** です（「複製」で自分の作品にしてから編集・保存）

## サンプル素材（NEW）
- サンプルは **DBではなくリポジトリ内JSON** に保存します（GitHub管理）
- ギャラリーの **「サンプル」** から一覧を開き、**「コピーして編集」** を押すと
  - サンプルを複製して **自分の作品（ownerId付き）** として保存できます
  - サンプルそのものはDBに保存されません（コピーだけが保存されます）
- サンプルデータ: `src/pages/pixel-art-maker/samples.json`
- 追加/更新用スクリプト: `scripts/generate_pixel_art_samples.mjs`

## 画面の流れ（UI）
- **最初はギャラリー**が表示されます
- ギャラリーには **検索 / 並び替え** があり、作品を探しやすくしています
- **新規作成**: ギャラリーの「＋ 新規作成」→ モーダルで「なまえ」 → 編集画面へ（16×16で開始）
- **既存編集**: ギャラリーの「編集」→ 編集画面へ（作品カードを開いてもOK）
- 編集画面には「ギャラリーへ戻る」があり、保存済み一覧はギャラリー側に集約しています

### 編集画面（ツールバー）
- 画面下に **アイコンのツールバー**（横スクロール可）があり、ここから編集します
- **色**: ツールボックスに **基本24色 + 最近使った8色** を常時表示
  - さらに「色」ボタンから、フルカラー（カラーピッカー）や拡張パレットも使えます
- **キャンバス拡張**: 「横+16」「縦+16」で、描いた内容はそのままにキャンバスが広がります（最大256）
- **キャンバス縮小（NEW）**: 「横-16」「縦-16」で、右/下側を削ってキャンバスを縮められます
  - 削る範囲にピクセルがある場合は警告が出ます（消える可能性あり）
 - **複製/削除**: 作品の **複製/削除はギャラリー側** に集約しました（編集画面は未保存の「削除（破棄）」のみ）

## 実装メモ（設計）
- **エントリ**: `index.html`
- **メインロジック**: `script.js`
- **スタイル**: `style.css`
- **保存API**: `src/js/pixelAssets.js`

### データ形式（ローカル）
`PixelAsset`（IndexedDBの `pixel_assets` ストア）:
- `id`: string（assetId）
- `ownerId`: string（作成者player.id。ギャラリーは共通だが、編集可否の判定に使う）
- `kind`: `"character" | "object" | "tile"`（※ UIでは区別しないが、互換性のため保持）
- `width/height`: number
- `pixels`: `Uint32Array`（`0xAARRGGBB`、`0` は透明）
- `createdAt/updatedAt`: ISO文字列

### データ形式（Supabase / DB）
テーブル:
- `pixel_assets`: 作品メタデータ（1作品=1行）
- `pixel_asset_frames`: フレームデータ（1コマ=1行、`frame_index` で管理）

※ 将来の「歩行/攻撃など複数フレーム」追加は `pixel_asset_frames` に行を増やすだけで拡張できます。

## 既知の課題 / TODO
- 既存作品の **縮小**（トリミング/切り抜き）
- 塗りつぶし/直線/矩形などツール追加
- 色パレット（最近使った色、固定パレット）※対応済み
- Supabaseへの同期（複数端末共有、公開/非公開）
- 他ゲーム側で `assetId` を指定して読み込む仕組み（キャラ選択UIなど）

