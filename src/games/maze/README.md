...
## 概要
最大32×32マスの迷路を「ペイント」して保存し、その迷路を簡易3D（レイキャスト）で遊べるゲームです。
...
- **スマホ/タブレット**: 左下のスティック（ドラッグ）で移動、画面ドラッグ（左右）で向き変更
- **タイム**: 右上にミニマップ、上部にタイム（⏱️）と自己ベスト（🏆）が出ます。

## 構成
- `index.html`: UI（タブ、キャンバス、ボタン）
- `game.js`: エディタ（32×32ペイント）と3Dプレイ（レイキャスト）

## 壁の見た目（NEW）
- プレイ画面で **壁の種類** を選べます
  - **単色**: 色を1つ選んで壁に適用
  - **ドット絵**: 「ドット絵メーカー」で作った **地形（tile）** を壁テクスチャとして使います
