...
- **配置選択（setup）**:
  ...
  - 「新しく作る」で、空盤から作れます
- **対局**:
  - 自分の駒を **ドラッグ＆ドロップ** で移動（ドラッグ開始で行けるマスが色つき表示）
  - もちろん **タップ → 行き先タップ** でも移動できます
  - **移動できない場所** にドロップしたり、**元の場所** に戻した場合はキャンセル扱い（移動しません）
  - 相手の駒の上に移動すると取り、**持ち駒** に入ります
  - 持ち駒も **ドラッグ＆ドロップ** で打てます（空きマスが色つき表示）
  - **長押し** で駒の説明を表示（図鑑と同じ情報）
  - 駒を置いた/取ったときに **効果音** と **軽いエフェクト** が出ます
- **配置編集**:
