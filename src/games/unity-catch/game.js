import { getCurrentPlayer } from '../../js/auth.js';
import { saveScore, getRankings } from '../../js/score.js';
import { avatarToHtml } from '../../js/avatar.js';

const GAME_ID = 'unity-catch';

const canvas = document.getElementById('gameCanvas');
const ctx = canvas.getContext('2d');

const scoreEl = document.getElementById('score');
const livesEl = document.getElementById('lives');
const startOverlay = document.getElementById('startOverlay');
const resultOverlay = document.getElementById('resultOverlay');
const startBtn = document.getElementById('startBtn');

const BASE_W = 360;
const BASE_H = 640;

const state = {
  running: false,
  rafId: 0,
  score: 0,
  lives: 3,
  time: 0,
  spawnTimer: 0,
  spawnEvery: 0.72, // seconds
  fallSpeed: 145, // px/sec
  gemChance: 0.12,
  heartChance: 0.04,
  bombChance: 0.16
};

const input = {
  left: false,
  right: false,
  pointerDown: false,
  pointerX: BASE_W / 2
};

// Unity(など)から書き出した画像を /public/games/unity-catch/assets/ に置けば置換できる
const ASSET_BASE = '/games/unity-catch/assets';
const images = {
  bg: makeImage(`${ASSET_BASE}/bg.png`),
  player: makeImage(`${ASSET_BASE}/player.png`),
  coin: makeImage(`${ASSET_BASE}/coin.png`),
  gem: makeImage(`${ASSET_BASE}/gem.png`),
  bomb: makeImage(`${ASSET_BASE}/bomb.png`),
  heart: makeImage(`${ASSET_BASE}/heart.png`)
};

const player = {
  x: BASE_W / 2,
  y: BASE_H - 86,
  w: 78,
  h: 78,
  speed: 290
};

/** @type {{x:number,y:number,r:number,vy:number,type:'coin'|'gem'|'bomb'|'heart',value:number}[]} */
let drops = [];

function makeImage(src) {
  const img = new Image();
  img.src = src;
  return img;
}

function clamp(v, min, max) {
  return Math.max(min, Math.min(max, v));
}

function rand(min, max) {
  return Math.random() * (max - min) + min;
}

function setLivesText() {
  livesEl.textContent = '❤️'.repeat(Math.max(0, state.lives));
}

function resetGame() {
  state.running = false;
  state.score = 0;
  state.lives = 3;
  state.time = 0;
  state.spawnTimer = 0;
  state.spawnEvery = 0.72;
  state.fallSpeed = 145;
  state.gemChance = 0.12;
  state.heartChance = 0.04;
  state.bombChance = 0.16;

  drops = [];
  player.x = BASE_W / 2;
  scoreEl.textContent = '0';
  setLivesText();
}

function resizeCanvas() {
  // Keep internal coordinate system stable, but render crisp on HiDPI.
  const dpr = Math.min(window.devicePixelRatio || 1, 2);
  canvas.width = Math.round(BASE_W * dpr);
  canvas.height = Math.round(BASE_H * dpr);
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
}

function toCanvasX(clientX) {
  const rect = canvas.getBoundingClientRect();
  const t = (clientX - rect.left) / rect.width;
  return clamp(t * BASE_W, 0, BASE_W);
}

function chooseType() {
  // Priority: heart (rare), gem, bomb, else coin
  const r = Math.random();
  if (r < state.heartChance) return 'heart';
  if (r < state.heartChance + state.gemChance) return 'gem';
  if (r < state.heartChance + state.gemChance + state.bombChance) return 'bomb';
  return 'coin';
}

function spawnDrop() {
  const type = chooseType();
  const r = type === 'gem' ? rand(12, 18) : type === 'heart' ? rand(12, 16) : rand(12, 18);
  const x = rand(18 + r, BASE_W - 18 - r);
  const y = -30;

  const value = type === 'gem' ? 5 : type === 'coin' ? 1 : 0;
  const speedJitter = rand(0.92, 1.18);

  drops.push({
    x,
    y,
    r,
    vy: state.fallSpeed * speedJitter,
    type,
    value
  });
}

function intersectsCircleRect(cx, cy, cr, rx, ry, rw, rh) {
  const closestX = clamp(cx, rx, rx + rw);
  const closestY = clamp(cy, ry, ry + rh);
  const dx = cx - closestX;
  const dy = cy - closestY;
  return dx * dx + dy * dy <= cr * cr;
}

function pathRoundRect(x, y, w, h, r) {
  const rr = clamp(r, 0, Math.min(w, h) / 2);
  if (typeof ctx.roundRect === 'function') {
    ctx.roundRect(x, y, w, h, rr);
    return;
  }
  // Fallback for older browsers
  ctx.moveTo(x + rr, y);
  ctx.arcTo(x + w, y, x + w, y + h, rr);
  ctx.arcTo(x + w, y + h, x, y + h, rr);
  ctx.arcTo(x, y + h, x, y, rr);
  ctx.arcTo(x, y, x + w, y, rr);
  ctx.closePath();
}

function drawBackground() {
  ctx.clearRect(0, 0, BASE_W, BASE_H);

  if (images.bg.complete && images.bg.naturalWidth > 0) {
    // cover
    const iw = images.bg.naturalWidth;
    const ih = images.bg.naturalHeight;
    const s = Math.max(BASE_W / iw, BASE_H / ih);
    const w = iw * s;
    const h = ih * s;
    ctx.drawImage(images.bg, (BASE_W - w) / 2, (BASE_H - h) / 2, w, h);
    return;
  }

  // Fallback: space-y gradient + stars
  const g = ctx.createLinearGradient(0, 0, 0, BASE_H);
  g.addColorStop(0, '#0ea5e9');
  g.addColorStop(0.35, '#0b1022');
  g.addColorStop(1, '#020617');
  ctx.fillStyle = g;
  ctx.fillRect(0, 0, BASE_W, BASE_H);

  ctx.globalAlpha = 0.25;
  ctx.fillStyle = '#ffffff';
  for (let i = 0; i < 32; i++) {
    const x = (i * 47 + (state.time * 18) % (BASE_W + 150)) % BASE_W;
    const y = (i * 71 + (state.time * 12) % (BASE_H + 200)) % BASE_H;
    ctx.fillRect(x, y, i % 3 === 0 ? 2 : 1, i % 4 === 0 ? 2 : 1);
  }
  ctx.globalAlpha = 1;
}

function drawPlayer() {
  const rx = player.x - player.w / 2;
  const ry = player.y - player.h / 2;

  if (images.player.complete && images.player.naturalWidth > 0) {
    ctx.drawImage(images.player, rx, ry, player.w, player.h);
    return;
  }

  // Fallback: cute basket-like
  ctx.save();
  ctx.translate(player.x, player.y);
  ctx.fillStyle = '#f59e0b';
  ctx.beginPath();
  pathRoundRect(-player.w * 0.44, -player.h * 0.18, player.w * 0.88, player.h * 0.55, 14);
  ctx.fill();
  ctx.strokeStyle = 'rgba(0,0,0,0.25)';
  ctx.lineWidth = 3;
  ctx.stroke();

  ctx.strokeStyle = 'rgba(255,255,255,0.45)';
  ctx.lineWidth = 5;
  ctx.beginPath();
  ctx.arc(0, -player.h * 0.06, player.w * 0.32, Math.PI * 1.05, Math.PI * 1.95);
  ctx.stroke();
  ctx.restore();
}

function drawDrop(d) {
  const img =
    d.type === 'coin'
      ? images.coin
      : d.type === 'gem'
        ? images.gem
        : d.type === 'heart'
          ? images.heart
          : images.bomb;

  if (img.complete && img.naturalWidth > 0) {
    const s = d.r * 2.2;
    ctx.drawImage(img, d.x - s / 2, d.y - s / 2, s, s);
    return;
  }

  // Fallback shapes
  if (d.type === 'coin') {
    ctx.fillStyle = '#fcd34d';
    ctx.beginPath();
    ctx.arc(d.x, d.y, d.r, 0, Math.PI * 2);
    ctx.fill();
    ctx.globalAlpha = 0.25;
    ctx.fillStyle = '#ffffff';
    ctx.beginPath();
    ctx.arc(d.x - d.r * 0.25, d.y - d.r * 0.25, d.r * 0.4, 0, Math.PI * 2);
    ctx.fill();
    ctx.globalAlpha = 1;
    return;
  }

  if (d.type === 'gem') {
    ctx.fillStyle = '#60a5fa';
    ctx.beginPath();
    ctx.moveTo(d.x, d.y - d.r);
    ctx.lineTo(d.x + d.r * 0.9, d.y);
    ctx.lineTo(d.x, d.y + d.r);
    ctx.lineTo(d.x - d.r * 0.9, d.y);
    ctx.closePath();
    ctx.fill();
    return;
  }

  if (d.type === 'heart') {
    ctx.fillStyle = '#fb7185';
    ctx.font = `bold ${Math.max(14, d.r * 1.7)}px Outfit, sans-serif`;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('❤️', d.x, d.y + 1);
    return;
  }

  // bomb
  ctx.fillStyle = '#111827';
  ctx.beginPath();
  ctx.arc(d.x, d.y, d.r, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = '#fb7185';
  ctx.font = `bold ${Math.max(14, d.r * 1.7)}px Outfit, sans-serif`;
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText('💣', d.x, d.y + 1);
}

function step(dt) {
  if (!state.running) return;

  state.time += dt;

  // Difficulty ramp
  state.fallSpeed = 145 + Math.min(260, state.time * 1.9);
  state.spawnEvery = 0.72 - Math.min(0.34, state.time / 260);
  state.bombChance = 0.16 + Math.min(0.12, state.time / 2400);
  state.gemChance = 0.12 + Math.min(0.08, state.time / 2200);
  state.heartChance = 0.04 + Math.min(0.02, state.time / 3000);

  // Spawn
  state.spawnTimer += dt;
  while (state.spawnTimer >= state.spawnEvery) {
    state.spawnTimer -= state.spawnEvery;
    spawnDrop();
  }

  // Player movement
  let vx = 0;
  if (input.left) vx -= 1;
  if (input.right) vx += 1;

  if (input.pointerDown) {
    const dx = input.pointerX - player.x;
    player.x += clamp(dx, -player.speed * dt, player.speed * dt);
  } else if (vx !== 0) {
    player.x += vx * player.speed * dt;
  }
  player.x = clamp(player.x, 20, BASE_W - 20);

  // Update drops
  const rx = player.x - player.w / 2;
  const ry = player.y - player.h / 2;

  for (let i = drops.length - 1; i >= 0; i--) {
    const d = drops[i];
    d.vy = state.fallSpeed * clamp(d.vy / Math.max(1, state.fallSpeed), 0.9, 1.2);
    d.y += d.vy * dt;

    // Catch
    if (intersectsCircleRect(d.x, d.y, d.r, rx, ry, player.w, player.h)) {
      drops.splice(i, 1);
      if (d.type === 'bomb') {
        state.lives -= 1;
        setLivesText();
      } else if (d.type === 'heart') {
        state.lives = Math.min(5, state.lives + 1);
        setLivesText();
      } else {
        state.score += d.value;
        scoreEl.textContent = String(state.score);
      }
      continue;
    }

    // Missed
    if (d.y - d.r > BASE_H + 10) {
      drops.splice(i, 1);
      // coin/gem missed: no penalty
    }
  }

  if (state.lives <= 0) {
    endGame();
  }
}

function render() {
  drawBackground();
  for (const d of drops) drawDrop(d);
  drawPlayer();
}

let lastTs = 0;
function loop(ts) {
  const t = ts / 1000;
  const dt = lastTs ? Math.min(0.033, t - lastTs) : 0;
  lastTs = t;

  step(dt);
  render();

  if (state.running) state.rafId = requestAnimationFrame(loop);
}

function startGame() {
  resetGame();
  state.running = true;
  lastTs = 0;
  startOverlay.style.display = 'none';
  resultOverlay.style.display = 'none';
  requestAnimationFrame(loop);
}

function formatRankingHtml(rankings) {
  if (!rankings.length) return '<div style="opacity:0.9;">まだスコアがありません</div>';

  return rankings
    .map(
      (r, i) => `
      <div style="display:flex; justify-content:space-between; width:100%; padding:5px; border-bottom:1px solid rgba(255,255,255,0.2);">
        <div style="display:flex; align-items:center; gap:8px;">
          <span style="font-weight:bold; width:20px;">${i + 1}.</span>
          <span style="display:inline-flex; align-items:center;">${avatarToHtml(r.avatar, { size: 24, className: 'ng-avatar', alt: '' })}</span>
          <span style="overflow:hidden; text-overflow:ellipsis; max-width:140px; white-space:nowrap;">${r.name}</span>
        </div>
        <span style="font-weight:bold;">${r.score}点</span>
      </div>
    `
    )
    .join('');
}

async function endGame() {
  state.running = false;
  cancelAnimationFrame(state.rafId);

  // Save score (best effort)
  const playerInfo = getCurrentPlayer();
  if (playerInfo) {
    await saveScore(GAME_ID, playerInfo.id, state.score);
  }
  const rankings = await getRankings(GAME_ID);

  resultOverlay.innerHTML = `
    <div class="overlay-card">
      <h1>おしまい！</h1>
      <p style="font-size: 2rem; font-weight: 900;">スコア: ${state.score}</p>

      <div style="background: rgba(255,255,255,0.15); padding: 15px; border-radius: 12px; margin: 15px 0; width: 100%; text-align: left;">
        <h3 style="text-align:center; margin-bottom: 10px; color: white;">🏆 ランキング</h3>
        ${formatRankingHtml(rankings)}
      </div>

      <button class="btn-primary" id="retryBtn">もういっかい</button>
      <a href="../../pages/portal/portal.html" style="color: white; margin-top: 18px; display: block;">&larr; ほかのゲーム</a>
    </div>
  `;
  resultOverlay.style.display = 'flex';
  document.getElementById('retryBtn')?.addEventListener('click', startGame);
}

// Input
document.addEventListener('keydown', (e) => {
  if (e.code === 'ArrowLeft') input.left = true;
  if (e.code === 'ArrowRight') input.right = true;
  if (e.code === 'Space') {
    if (!state.running && resultOverlay.style.display === 'flex') startGame();
    e.preventDefault();
  }
});
document.addEventListener('keyup', (e) => {
  if (e.code === 'ArrowLeft') input.left = false;
  if (e.code === 'ArrowRight') input.right = false;
});

function onPointerDown(clientX) {
  input.pointerDown = true;
  input.pointerX = toCanvasX(clientX);
}
function onPointerMove(clientX) {
  if (!input.pointerDown) return;
  input.pointerX = toCanvasX(clientX);
}
function onPointerUp() {
  input.pointerDown = false;
}

canvas.addEventListener(
  'pointerdown',
  (e) => {
    onPointerDown(e.clientX);
    canvas.setPointerCapture(e.pointerId);
    e.preventDefault();
  },
  { passive: false }
);
canvas.addEventListener(
  'pointermove',
  (e) => {
    onPointerMove(e.clientX);
    e.preventDefault();
  },
  { passive: false }
);
canvas.addEventListener(
  'pointerup',
  (e) => {
    onPointerUp();
    e.preventDefault();
  },
  { passive: false }
);
canvas.addEventListener(
  'pointercancel',
  (e) => {
    onPointerUp();
    e.preventDefault();
  },
  { passive: false }
);

// Boot
resizeCanvas();
window.addEventListener('resize', resizeCanvas);
resetGame();
render();
startBtn.addEventListener('click', startGame);

