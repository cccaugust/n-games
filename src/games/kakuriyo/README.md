...
- **セーブ/ロード**: 階層移動時に自動セーブ。死亡・クリア時にセーブ削除
- **ダッシュ**: 最後に移動した方向へ自動移動（敵発見・分岐点で停止）
- **ステータス表示**: メニューから詳細なステータスパネル表示
- **ボス戦**: B100Fで「輪廻守」との対決。HPに応じて3段階の形態変化
- **モンスターハウス**: B10F以降で出現。大量の敵とアイテムが配置される特殊部屋
