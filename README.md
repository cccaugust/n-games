...
4.  **ドット絵メーカー（準備中）**:
    *   キャラ/モノ/地形のドット絵を作成・保存し、将来いろんなゲームで使える共通アセットにする（保存先: Supabase DB）。
...
初見のプレイヤーが混乱しないよう、画面上に **開発者向け/AIの制作メモ（仕様変更ログ、TODO、注意書き等）** を表示しない方針です。  
必要な開発情報は各ゲームの `README.md` やデバッグページに分離し、UIに出す場合はプレイヤー向けの短い説明に言い換えます（詳細は `.cursor/rules/10-frontend-games.mdc`）。
